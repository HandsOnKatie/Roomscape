...
/* ================= ROOMSCAPE ROOMS PHASE 1 (2026-07-11) =================
   Multi-room phase 1, appended to conductor.js by "APPLY ROOMS PHASE1.bat".
   Adds:
     - HA bridge domains: script + cover (for playroom mode scripts & blinds)
     - state.rooms  — live mode per non-dining room (persisted + broadcast;
       frames ignore it, so the wall is untouched)
     - GET  /api/rooms                       → rooms config + per-room modes + live state
     - POST /api/room/<roomId>/mode/<modeId> → fire that mode's HA script, record live
     - POST /api/room/<roomId>/off           → fire the room's isOff profile, clear live
   Rooms config lives in settings.rooms; a room mode is a profile with
   { room:'playroom', script:'script.playroom_gaming_mode', hidden:true }.
   Deliberately does NOT call bump()/applyProfile — no dining side-effects.   */
;(function(){
  try{
    ...
    server.prependListener('request', function(req, res){
      ...
      if (p !== '/api/rooms' && p.indexOf('/api/room/') !== 0) return;   // not ours — main handler proceeds
      // Ours: snapshot the real response fns, then neuter res so the main
      // handler's fall-through 404 writes go nowhere (it runs right after us).
      ...
      try{
        ...
        var seg = p.split('/');
        var roomId = decodeURIComponent(seg[3] || ''), verb = seg[4] || '', modeId = decodeURIComponent(seg[5] || '');
        if (!roomsCfg().some(function(r){ return r.id === roomId; })) return out(404, { ok: false, error: 'unknown room: ' + roomId });
        ...
        function fire(scriptEnt, done){
          if (!scriptEnt) return done(null);                      // stateful-only mode
          if (!haOn()) return done('HA not configured');
          var called = false;
          function once(e){ if (!called){ called = true; done(e); } }
          haCall('script', 'turn_on', { entity_id: scriptEnt }, function(err, code){
            if (err) return once(String(err && err.message || err));
            if (code && code >= 400) return once('HA returned ' + code);
            once(null);
          });
          setTimeout(function(){ once('HA timeout'); }, 10000);
        }
        if (verb === 'mode' && modeId){
          var pr = profiles[modeId];
          if (!pr || pr.room !== roomId) return out(404, { ok: false, error: 'unknown mode ' + modeId + ' for room ' + roomId });
          return fire(pr.script, function(err){
            if (err) return out(502, { ok: false, error: err });
            setLive(pr.isOff ? null : modeId);
            out(200, { ok: true, room: roomId, mode: pr.isOff ? null : modeId });
          });
        }
        if (verb === 'off'){
          var offKey = Object.keys(profiles).find(function(k){ return profiles[k] && profiles[k].room === roomId && profiles[k].isOff; });
          return fire(offKey ? profiles[offKey].script : null, function(err){
            if (err) return out(502, { ok: false, error: err });
            setLive(null);
            out(200, { ok: true, room: roomId, mode: null });
          });
        }
        return out(404, { ok: false, error: 'unknown rooms endpoint' });
      }catch(e){ return out(500, { ok: false, error: String(e && e.message || e) }); }
    });
    console.log('[rooms] phase-1 active: HA domains +script +cover · GET /api/rooms · POST /api/room/<id>/mode|off');
  }catch(e){ console.error('[rooms] phase-1 patch failed to initialise:', e && e.message); }
})();

/* ================= ROOMSCAPE ROOMS PHASE 2 (2026-07-11) =================
   Native room modes + per-mode Music Assistant playlists.
   Appended AFTER the phase-1 patch by "APPLY ROOMS PHASE2.bat".
   - profile.haActions[] — sequential HA service steps run by the Conductor
     ({action:'light.turn_on', entity:[...], data:{...}, delayMs, gapMs});
     falls back to profile.script when absent (dance/music keep their HA
     scripts — they end in infinite colour loops that belong in HA).
   - profile.music — MA playlist name played on the room's player
     (settings.rooms[].ma.player, matched against MA players/all) when the
     mode starts; stopped on the room's off. Dining music untouched.
   - GET /api/ma/players — list MA players (debug/config aid).
   - REPLACES the phase-1 request listener (removes it, installs its own),
     because phase 1 would otherwise still fire the old scripts in parallel. */
;(function(){
  try{
    HA_DOMAINS.script = 1; HA_DOMAINS.cover = 1;   // idempotent, keep true if phase 1 text is ever removed
    if (!state.rooms || typeof state.rooms !== 'object') state.rooms = {};
    var FALLBACK_ROOMS = [
      { id: 'dining',   name: 'Dining Room', icon: '🍽' },
      { id: 'playroom', name: 'Playroom',    icon: '🎮' }
    ...
    function log2(m){ try{ console.log('[rooms2] ' + m); }catch(e){} }

    /* ---------- swap out the phase-1 listener ---------- */
    try{
      var ls = server.listeners('request');
      if (ls.length > 1){ server.removeListener('request', ls[0]); log2('phase-1 listener removed (' + ls.length + ' -> ' + server.listeners('request').length + ')'); }
      else { log2('WARNING: expected phase-1 listener not found; continuing'); }
    }catch(e){ log2('listener swap failed: ' + (e && e.message)); }

    /* ---------- Music Assistant helpers ---------- */
    function maItems2(r){ return Array.isArray(r) ? r : ((r && r.items) || []); }
    function maRoomPlayer(room, cb){                       // cb(err, playerId, allPlayers)
      var want = room && room.ma && room.ma.player;
      if (!want) return cb(null, null);
      maCall('players/all', {}, function(e, r){
        if (e) return cb(e);
        var list = maItems2(r).map(function(x){ return { id: x.player_id || x.queue_id || x.id, name: String(x.display_name || x.name || '') }; });
        var n = String(want).toLowerCase();
        var hit = list.find(function(x){ return x.name.toLowerCase() === n; }) ||
                  list.find(function(x){ return x.name.toLowerCase().indexOf(n) !== -1; });
        cb(null, hit ? hit.id : null, list);
      });
    }
    function roomMusicPlay(room, label){
      if (!label || label === '—') return;
      if (!((settings.music || {}).url)) return log2('music: MA url not set');
      maRoomPlayer(room, function(e, pid){
        if (e || !pid) return log2('music: no MA player for ' + room.id + (e ? ' (' + e.message + ')' : ' (set settings.rooms[].ma.player — see /api/ma/players)'));
        maCall('music/playlists/library_items', { limit: 500 }, function(e2, r2){
          if (e2) return log2('music: playlists failed: ' + e2.message);
          var items = maItems2(r2), n = String(label).toLowerCase();
          var pl = items.find(function(x){ return String(x.name || '').toLowerCase() === n; }) ||
                   items.find(function(x){ return String(x.name || '').toLowerCase().indexOf(n) !== -1; });
          if (!pl) return log2('music: playlist not found: ' + label);
          maCall('player_queues/play_media', { queue_id: pid, media: pl.uri || pl.name, option: 'replace' }, function(e3){
            log2(e3 ? ('music: play failed: ' + e3.message) : ('music: "' + pl.name + '" → ' + room.id));
          });
        });
      });
    }
    function roomMusicStop(room){
      maRoomPlayer(room, function(e, pid){
        if (e || !pid) return;
        maCall('players/cmd/stop', { player_id: pid }, function(e2){
          if (e2) return maCall('player_queues/stop', { queue_id: pid }, function(){ log2('music: stopped (queue) ' + room.id); });
          log2('music: stopped ' + room.id);
        });
      });
    }
    ...
    function activate(pr, cb){
      if (Array.isArray(pr.haActions) && pr.haActions.length){
        ...
        return runHaActions(pr.haActions, cb);
      }
      if (pr.script){
        if (!haOn()) return cb('HA not configured');
        var called = false;
        function once(e){ if (!called){ called = true; cb(e); } }
      ...
      cb(null);   // stateful-only mode
    }

    /* ---------- request listener (same routes as phase 1 + /api/ma/players) ---------- */
    server.prependListener('request', function(req, res){
      var p; try{ p = new URL(req.url, 'http://localhost').pathname; }catch(e){ return; }
      if (p !== '/api/rooms' && p !== '/api/ma/players' && p.indexOf('/api/room/') !== 0) return;
      var W = res.writeHead.bind(res), E = res.end.bind(res);
      res.writeHead = res.setHeader = function(){ return res; };
      res.write = res.end = function(){ return true; };
      function out(code, obj){
      ...
      try{
        if (p === '/api/ma/players'){
          return maCall('players/all', {}, function(e, r){
            ...
            out(200, { ok: true, players: maItems2(r).map(function(x){ return { id: x.player_id || x.id, name: x.display_name || x.name || '', available: x.available !== false }; }) });
          });
        }
        if (p === '/api/rooms'){
          var modes = {};
        ...
        if (verb === 'mode' && modeId){
          var pr = profiles[modeId];
          if (!pr || pr.room !== roomId) return out(404, { ok: false, error: 'unknown mode ' + modeId + ' for room ' + roomId });
          return activate(pr, function(err){
            if (err) return out(502, { ok: false, error: err });
            setLive(pr.isOff ? null : modeId);
            if (pr.isOff) roomMusicStop(room);
            else if (pr.music) roomMusicPlay(room, pr.music);
            out(200, { ok: true, room: roomId, mode: pr.isOff ? null : modeId, native: !!(pr.haActions && pr.haActions.length), music: pr.music || null });
          });
        }
        if (verb === 'off'){
          var offKey = Object.keys(profiles).find(function(k){ return profiles[k] && profiles[k].room === roomId && profiles[k].isOff; });
          var offPr = offKey ? profiles[offKey] : {};
          return activate(offPr, function(err){
            if (err) return out(502, { ok: false, error: err });
            setLive(null);
            roomMusicStop(room);
            out(200, { ok: true, room: roomId, mode: null });
    ...
    log2('phase-2 active: haActions + room music (MA) + /api/ma/players');
  }catch(e){ console.error('[rooms2] phase-2 patch failed to initialise:', e && e.message); }
})();

/* ================= ROOMSCAPE ROOMS PHASE 2.1 (2026-07-11) =================
   Robustness fix: haActions step timeouts/errors are WARNINGS, not failures.
   Phase 2 aborted the whole activation on any step error (a slow/unavailable
   Govee froze the response, left music running and state stale). Now the
   room state ALWAYS updates, music ALWAYS starts/stops, and step errors are
   returned in a `warnings` field + logged. Replaces the phase-2 listener.  */
;(function(){
  try{
    HA_DOMAINS.script = 1; HA_DOMAINS.cover = 1;
    if (!state.rooms || typeof state.rooms !== 'object') state.rooms = {};
    ...
    function log2(m){ try{ console.log('[rooms2.1] ' + m); }catch(e){} }
    try{
      var ls = server.listeners('request');
      if (ls.length > 1){ server.removeListener('request', ls[0]); log2('previous rooms listener removed'); }
      else { log2('WARNING: previous rooms listener not found'); }
    }catch(e){ log2('listener swap failed: ' + (e && e.message)); }

    function maItems2(r){ return Array.isArray(r) ? r : ((r && r.items) || []); }
    function maRoomPlayer(room, cb){
      var want = room && room.ma && room.ma.player;
      if (!want) return cb(null, null);
      maCall('players/all', {}, function(e, r){
        if (e) return cb(e);
    ...
    function roomMusicPlay(room, label){
      ...
      if (!((settings.music || {}).url)) return log2('music: MA url not set');
      maRoomPlayer(room, function(e, pid){
        if (e || !pid) return log2('music: no MA player for ' + room.id);
        maCall('music/playlists/library_items', { limit: 500 }, function(e2, r2){
          if (e2) return log2('music: playlists failed: ' + e2.message);
          var items = maItems2(r2), n = String(label).toLowerCase();
          var pl = items.find(function(x){ return String(x.name || '').toLowerCase() === n; }) ||
    ...
    function roomMusicStop(room){
      maRoomPlayer(room, function(e, pid){
        if (e || !pid) return;
        maCall('players/cmd/stop', { player_id: pid }, function(e2){
          if (e2) return maCall('player_queues/stop', { queue_id: pid }, function(){});
          log2('music: stopped ' + room.id);
        });
      });
    }
    ...
    function activate(pr, cb){                             // cb(fatalErr, warnings)
      if (Array.isArray(pr.haActions) && pr.haActions.length){
        if (!haOn()) return cb('HA not configured');
        return runHaActions(pr.haActions, function(warn){ cb(null, warn); });   // step errors are warnings
      }
      if (pr.script){
        if (!haOn()) return cb('HA not configured');
        var called = false;
        ...
        haCall('script', 'turn_on', { entity_id: pr.script }, function(err, code){
          if (err) return once(null, 'script: ' + String(err.message || err));   // fired-and-unsure = warning
          if (code && code >= 400) return once('HA returned ' + code);           // HA rejected = real failure
          once(null);
        });
      ...
      try{
        ...
        if (verb === 'mode' && modeId){
          ...
          return activate(pr, function(err, warn){
            ...
            if (warn) log2('mode ' + modeId + ' warnings: ' + warn);
        ...
        if (verb === 'off'){
          ...
          return activate(offPr, function(err, warn){
            setLive(null);                                   // state + music ALWAYS settle on off
            roomMusicStop(room);
            if (err || warn) log2('off ' + roomId + ' issues: ' + (err || warn));
    ...
    log2('phase-2.1 active: warnings-not-failures + guaranteed off');
  }catch(e){ console.error('[rooms2.1] patch failed to initialise:', e && e.message); }
})();

/* ================= ROOMSCAPE ROOMS PHASE 2.2 (2026-07-11) =================
   Per-room music status + transport. Small ADDITIVE listener — only handles
   /api/room/<id>/music[...]; the 2.1 listener passes these through harmlessly
   (its 404 write is neutered because this listener responds first).
   - GET  /api/room/<id>/music        → { playing, title, artist }
   - POST /api/room/<id>/music/pause  → players/cmd/play_pause
   - POST /api/room/<id>/music/next   → players/cmd/next
   - POST /api/room/<id>/music/stop   → players/cmd/stop                     */
;(function(){
  try{
    function log3(m){ try{ console.log('[rooms2.2] ' + m); }catch(e){} }
    function roomsCfg22(){
      var r = settings && settings.rooms;
      return (Array.isArray(r) && r.length) ? r : [];
    }
    function maItems3(r){ return Array.isArray(r) ? r : ((r && r.items) || []); }
    function maPlayer3(room, cb){
      var want = room && room.ma && room.ma.player;
      if (!want) return cb(null, null);
      maCall('players/all', {}, function(e, r){
        if (e) return cb(e);
        var list = maItems3(r).map(function(x){ return { id: x.player_id || x.queue_id || x.id, name: String(x.display_name || x.name || '') }; });
        var n = String(want).toLowerCase();
        var hit = list.find(function(x){ return x.name.toLowerCase() === n; }) ||
                  list.find(function(x){ return x.name.toLowerCase().indexOf(n) !== -1; });
        cb(null, hit ? hit.id : null);
    ...
    server.prependListener('request', function(req, res){
      var p; try{ p = new URL(req.url, 'http://localhost').pathname; }catch(e){ return; }
      var m = p.match(/^\/api\/room\/([^\/]+)\/music(?:\/([a-z_]+))?$/);
      if (!m) return;
      var W = res.writeHead.bind(res), E = res.end.bind(res);
      res.writeHead = res.setHeader = function(){ return res; };
      res.write = res.end = function(){ return true; };
      ...
      try{
        var roomId = decodeURIComponent(m[1]), sub = m[2] || '';
        var room = roomsCfg22().find(function(r){ return r.id === roomId; });
        if (!room) return out(404, { ok: false, error: 'unknown room: ' + roomId });
        if (!(room.ma && room.ma.player)) return out(200, { ok: true, playing: false, configured: false });
        maPlayer3(room, function(e, pid){
          if (e) return out(502, { ok: false, error: e.message });
          if (!pid) return out(200, { ok: true, playing: false, configured: false });
          if (!sub){                                        // status
            ...
            return maCall('player_queues/all', {}, function(e2, r2){
              ...
              var q = maItems3(r2).find(function(x){ return x && (x.queue_id === pid || x.player_id === pid); });
              var cur = q && (q.current_item || q.current_media) || null;
              var md = cur && (cur.media_item || cur) || null;
              out(200, { ok: true, configured: true,
                playing: !!(q && String(q.state || '').toLowerCase() === 'playing'),
                title: (md && md.name) || '',
                artist: (md && md.artists && md.artists[0] && md.artists[0].name) || '' });
            });
          }
          if (req.method !== 'POST') return out(405, { ok: false, error: 'POST only' });
          ...
          maCall(cmds[sub], { player_id: pid }, function(e3){
    ...
    log3('phase-2.2 active: /api/room/<id>/music status + pause/next/stop');
  }catch(e){ console.error('[rooms2.2] patch failed to initialise:', e && e.message); }
})();

/* ================= ROOMSCAPE ROOMS PHASE 2.3 (2026-07-11) =================
   Richer room music + richer /api/rooms.
   - REPLACES the 2.2 music listener (leaving it would double-fire transport
     commands) with: GET /api/room/<id>/music (playing, paused, title, artist,
     image, volume) · POST .../music/play {uri|name} · pause|next|stop ·
     POST .../music/volume {volume:0-100}.
   - TAKES OVER GET /api/rooms (read-only; the 2.1 listener's copy responds
     into a neutered res) adding mode fields: image, variantOf, variantLabel. */
;(function(){
  try{
    function log4(m){ try{ console.log('[rooms2.3] ' + m); }catch(e){} }
    try{
      var ls = server.listeners('request');
      if (ls.length > 2){ server.removeListener('request', ls[0]); log4('2.2 music listener removed'); }
      else { log4('WARNING: expected [2.2, 2.1, main] listeners, found ' + ls.length + ' — continuing'); }
    }catch(e){ log4('listener swap failed: ' + (e && e.message)); }
    function roomsCfg23(){
      var r = settings && settings.rooms;
      return (Array.isArray(r) && r.length) ? r : [];
    }
    ...
    function player(room, cb){
      var want = room && room.ma && room.ma.player;
      if (!want) return cb(null, null);
      maCall('players/all', {}, function(e, r){
        if (e) return cb(e);
        var list = items(r).map(function(x){ return { id: x.player_id || x.queue_id || x.id, name: String(x.display_name || x.name || ''), volume: (x.volume_level != null ? x.volume_level : null) }; });
        var n = String(want).toLowerCase();
        var hit = list.find(function(x){ return x.name.toLowerCase() === n; }) ||
                  list.find(function(x){ return x.name.toLowerCase().indexOf(n) !== -1; });
        cb(null, hit || null);
    ...
    server.prependListener('request', function(req, res){
      ...
      try{
        if (p === '/api/rooms'){
          ...
          var rooms = roomsCfg23();
          return out(200, { ok: true, rooms: rooms.length ? rooms : [{id:'dining',name:'Dining Room',icon:'🍽'},{id:'playroom',name:'Playroom',icon:'🎮'}], modes: modes, state: state.rooms || {} });
        }
        var roomId = decodeURIComponent(m[1]), sub = m[2] || '';
        var room = roomsCfg23().find(function(r){ return r.id === roomId; });
        if (!room) return out(404, { ok: false, error: 'unknown room: ' + roomId });
        if (!(room.ma && room.ma.player)) return out(200, { ok: true, playing: false, configured: false });
        player(room, function(e, pl){
          if (e) return out(502, { ok: false, error: e.message });
    ...
    log4('phase-2.3 active: rooms+images/variants · music status+image+volume · play · volume');
  }catch(e){ console.error('[rooms2.3] patch failed to initialise:', e && e.message); }
})();
